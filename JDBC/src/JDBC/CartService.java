...
public class CartService {
	...
	public void addProduct(int productId, int quantity, double pricePerUnit) {
        String querry = "INSERT INTO Cart (userId, productId, quantity, totalPrice) VALUES (?, ?, ?, ?)";
	...
	public void updateProductQuantity(int productId, int newQuantity, double pricePerUnit) {
        String querry = "UPDATE Cart SET quantity = ?, totalPrice = ? WHERE userId = ? AND productId = ?";
        try (Connection conn = Database.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(querry)) {
    ...
    public List<CartItem> getCartProducts() {
        String querry = "SELECT * FROM Cart WHERE userId = ?";
    ...
    public boolean removeProduct(int productId) {
    	String querry = "DELETE FROM Cart WHERE userId = ? AND productId = ?";
    	try {
